...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    char buf_mem[512];
    ddriver_read(super.driver_fd, buf_mem, 512);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry target_dentry;
    memcpy(&target_dentry, buf_mem, sizeof(target_dentry));


    /* TODO: 填充filename */
    memcpy(filename, target_dentry.fname, sizeof(filename));
